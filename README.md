...
这里的 `persona` 不是目标读者，而是写作者本人。

例如可以这样设置：

```bash
npm run persona -- --text "一个资深的互联网运维工程师，擅长软件落地实施和维护，不讲空话和套话，只分享自己探索互联最新产品的过程，分享踩坑和落地经验，强调能上手就能用的方式。"
```

这类设定会在每次生成文章时自动读取，用来约束文章的语气、立场和表达方式。
